...
/** 编辑加工状态 */
const editStatusMap: Record<string, string> = {
  '0': '待编辑',
  '1': '编辑中',
  '2': '已完成',
};
...
async function loadFilterOptions() {
  try {
    const [yearRes, issueRes, columnRes] = (await Promise.all([
      publisherApi.getYearList({ pageNum: 1, pageSize: 100 }),
      publisherApi.getIssueList({ pageNum: 1, pageSize: 200 }),
      publisherApi.getColumnList({ pageNum: 1, pageSize: 200 }),
    ])) as any[];
    const years = (yearRes?.rows ?? [])
      .map((r: any) => r.year)
      .filter(Boolean);
    const issueYears = (issueRes?.rows ?? [])
      .map((r: any) => r.year)
      .filter(Boolean);
    yearOptions.value = [...new Set<number>([...years, ...issueYears])].sort(
      (a, b) => b - a,
    );
    columnOptions.value = [
      ...new Set<string>(
...
const advancedFields = [
  ...
  { value: 'editorName', label: '指定责编', type: 'text' },
  { value: 'status', label: '稿件阶段', type: 'status' },
  { value: 'editStatus', label: '处理中状态', type: 'editStatus' },
  { value: 'columnName', label: '栏目名称', type: 'text' },
  { value: 'editEndTime', label: '完成日期', type: 'range' },
] as const;
...
/** 表格 */
const tableData = ref<StatisticsApi.EditArticleCycleRow[]>([]);
...
const allColumns = [
  ...
  { key: 'statusName', title: '稿件阶段', dataIndex: 'statusName', width: 100 },
  { key: 'editorName', title: '指定责编', dataIndex: 'editorName', width: 100 },
  { key: 'editEndTime', title: '完成日期', dataIndex: 'editEndTime', width: 160 },
  { key: 'editStartTime', title: '选件时间', dataIndex: 'editStartTime', width: 160 },
  { key: 'year', title: '年份', dataIndex: 'year', width: 80 },
  { key: 'period', title: '期', dataIndex: 'period', width: 70 },
  { key: 'columnName', title: '栏目名称', dataIndex: 'columnName', width: 120 },
  {
    key: 'editArticleCycleDays',
    title: '加工周期',
    dataIndex: 'editArticleCycleDays',
    width: 90,
  },
];
...
function buildSearchParams(): StatisticsApi.EditArticleCycleListParams {
  const params: StatisticsApi.EditArticleCycleListParams = {
  ...
  if (filters.editStatus) params.editStatus = filters.editStatus;
...
async function loadTable() {
  ...
  try {
    const res = (await getEditArticleCycleList(buildSearchParams())) as any;
    tableData.value = res?.rows ?? [];
    pagination.total = res?.total ?? 0;
    selectedKeys.value = [];
  } catch {
    message.error('加载加工周期统计失败');
    tableData.value = [];
    pagination.total = 0;
  } finally {
...
function statusName(row: StatisticsApi.EditArticleCycleRow) {
...
async function onExport() {
  try {
    ...
    await exportEditArticleCycleList(params);
  } catch {
    message.error('导出失败');
  }
}
  ...
  <div class="p-4">
    ...
    <div v-show="advancedOpen" class="bg-card mb-3 rounded-md border px-3 py-3">
      ...
      >
        ...
        <a-select
          v-else-if="fieldType(row.field) === 'editStatus'"
          v-model:value="row.value"
          allow-clear
          placeholder="请选择处理状态"
          size="small"
        ...
        >
          <a-select-option
            v-for="(label, code) in editStatusMap"
            :key="code"
            :value="code"
          >
            {{ label }}
    ...
    <a-card :bordered="false" class="shadow-sm">
      ...
      <div class="mb-3 flex flex-wrap items-center gap-2">
        ...
        <a-select
          v-model:value="filters.editStatus"
          allow-clear
          placeholder="--处理中状态--"
          style="width: 140px"
          @change="onSearch"
      ...
      >
        <template #emptyText>
          <a-empty description="未查询到相关数据信息!" />
        </template>
        ...
        <template #bodyCell="{ column, record }">
          ...
          <template v-else-if="column.key === 'editorName'">
            {{ record.editorName || '-' }}
          </template>
          <template v-else-if="column.key === 'editEndTime'">
            {{ formatDateTime(record.editEndTime) }}
          </template>
          <template v-else-if="column.key === 'editStartTime'">
            {{ formatDateTime(record.editStartTime) }}
          </template>
          ...
          <template v-else-if="column.key === 'editArticleCycleDays'">
            {{ record.editArticleCycleDays ?? '-' }}
          </template>
        </template>
      </a-table>
