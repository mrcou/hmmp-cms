...
/**
 * 责编处理周期统计 —— 按稿件维度展示分配责编至终审结论的周期天数
...
import {
  exportEditProcessCycleList,
  getEditProcessCycleList,
} from '#/api/biz/statistics';

defineOptions({ name: 'StatisticsIntegrativeEditProcessCycle' });
...
const filters = reactive({
  ...
  editorStatus: undefined as string | undefined,
  searchField: 'fileNo' as 'authorNames' | 'editorName' | 'fileNo' | 'title',
  searchValue: '',
});
...
const advancedFields = [
  ...
  { value: 'assignTime', label: '分配时间', type: 'range' },
] as const;
...
/** 表格 */
const tableData = ref<StatisticsApi.EditProcessCycleRow[]>([]);
...
const allColumns = [
  ...
  { key: 'conclusionTime', title: '终止审稿', dataIndex: 'conclusionTime', width: 160 },
  { key: 'statusName', title: '稿件状态', dataIndex: 'statusName', width: 100 },
  {
    key: 'responsibleEditor',
    title: '责任编辑',
    dataIndex: 'responsibleEditor',
    width: 100,
  },
  ...
  {
    key: 'editProcessCycleDays',
    title: '责编处理周期',
    dataIndex: 'editProcessCycleDays',
    width: 120,
  },
];
...
function buildSearchParams(): StatisticsApi.EditProcessCycleListParams {
  const params: StatisticsApi.EditProcessCycleListParams = {
  ...
  else if (filters.editorStatus) params.status = filters.editorStatus;
...
async function loadTable() {
  ...
  try {
    const res = (await getEditProcessCycleList(buildSearchParams())) as any;
    tableData.value = res?.rows ?? [];
    pagination.total = res?.total ?? 0;
    selectedKeys.value = [];
  } catch {
    message.error('加载责编处理周期统计失败');
    tableData.value = [];
    pagination.total = 0;
  } finally {
...
function statusName(row: StatisticsApi.EditProcessCycleRow) {
  return row.conclusion || statusMap[row.status ?? ''] || '-';
}
...
async function onExport() {
  try {
    ...
    await exportEditProcessCycleList(params);
  } catch {
    message.error('导出失败');
  }
}
  ...
  <div class="p-4">
    ...
    <a-card :bordered="false" class="shadow-sm">
      ...
      <div class="mb-3 flex flex-wrap items-center gap-2">
        ...
        <a-select
          v-model:value="filters.editorStatus"
          allow-clear
          placeholder="--审理者状态--"
          style="width: 140px"
          @change="onSearch"
      ...
      >
        ...
        <template #bodyCell="{ column, record }">
          ...
          <template v-else-if="column.key === 'conclusionTime'">
            {{ record.conclusionTime || '-' }}
          </template>
          <template v-else-if="column.key === 'statusName'">
            {{ statusName(record) }}
          </template>
          <template v-else-if="column.key === 'responsibleEditor'">
            {{ record.responsibleEditor || record.editorName || '-' }}
          </template>
          ...
          <template v-else-if="column.key === 'editProcessCycleDays'">
            {{ record.editProcessCycleDays ?? '-' }}
          </template>
        </template>
      </a-table>
