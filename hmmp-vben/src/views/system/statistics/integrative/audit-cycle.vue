...
import { exportAuditCycleList, getAuditCycleList } from '#/api/biz/statistics';

defineOptions({ name: 'StatisticsAuditCycle' });
...
const filters = reactive({
  ...
  period: undefined as number | string | undefined,
...
const periodOptions = ref<Array<number | string>>([]);
  ...
  try {
    ...
    periodOptions.value = [
      ...new Set<number | string>(
        (issueRes?.rows ?? []).map((r: any) => r.period).filter(Boolean),
      ),
...
const advancedFields = [
  ...
  { value: 'receiveTime', label: '投稿时间', type: 'range' },
] as const;
...
/** 表格 */
const tableData = ref<StatisticsApi.AuditCycleRow[]>([]);
...
const allColumns = [
  ...
  { key: 'receiveTime', title: '投稿时间', dataIndex: 'receiveTime', width: 110 },
  ...
  { key: 'auditCycleDays', title: '审理周期', dataIndex: 'auditCycleDays', width: 90 },
];
...
function buildSearchParams(): StatisticsApi.AuditCycleListParams {
  const params: StatisticsApi.AuditCycleListParams = {
...
async function loadTable() {
  ...
  try {
    const res = (await getAuditCycleList(buildSearchParams())) as any;
    tableData.value = res?.rows ?? [];
    pagination.total = res?.total ?? 0;
    selectedKeys.value = [];
  } catch {
    message.error('加载审理周期统计失败');
    tableData.value = [];
    pagination.total = 0;
  } finally {
...
function statusName(row: StatisticsApi.AuditCycleRow) {
...
async function onExport() {
  try {
    ...
    await exportAuditCycleList(params);
  } catch {
    message.error('导出失败');
  }
}
    ...
    <a-card :bordered="false" class="shadow-sm">
      ...
      <div class="mb-3 flex flex-wrap items-center gap-2">
        ...
        <a-select
          ...
          @change="onSearch"
        >
          <a-select-option v-for="p in periodOptions" :key="p" :value="p">
            第{{ p }}期
          </a-select-option>
        </a-select>
        <a-select
      ...
      >
        ...
        <template #bodyCell="{ column, record }">
          ...
          <template v-else-if="column.key === 'auditCycleDays'">
            {{ record.auditCycleDays ?? '-' }}
          </template>
        </template>
      </a-table>
