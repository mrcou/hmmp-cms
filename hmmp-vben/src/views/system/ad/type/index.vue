...
const dataSource = ref<adApi.AdApi.AdType[]>([]);
const pagination = reactive({ current: 1, pageSize: 10, total: 0 });

const modalVisible = ref(false);
const modalTitle = ref('新增广告类型');
const formData = reactive<adApi.AdApi.AdType>({ typeName: '', typeCode: '', description: '', status: '0' });
...
async function fetchData() {
  loading.value = true;
  try {
    const res = await adApi.getAdTypeList({
...
function openCreateModal() {
  isEdit.value = false;
  modalTitle.value = '新增广告类型';
  Object.assign(formData, { typeName: '', typeCode: '', description: '', status: '0' });
  modalVisible.value = true;
}

function openEditModal(record: adApi.AdApi.AdType) {
  isEdit.value = true;
  modalTitle.value = '编辑广告类型';
...
async function handleSubmit() {
  ...
  try {
    if (isEdit.value) {
      await adApi.updateAdType({ ...formData });
      message.success('修改成功');
    } else {
      await adApi.createAdType({ ...formData });
...
async function handleDelete(record: adApi.AdApi.AdType) {
  await adApi.deleteAdType([record.typeId!]);
...
<template>
  <div class="p-4">
    <a-card :bordered="false" title="广告类型管理">
